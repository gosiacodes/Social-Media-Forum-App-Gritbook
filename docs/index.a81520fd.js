 ...
 * limitations under the License.
 */
ie("firebase","9.6.10","app");const zr=function(e=function(e="[DEFAULT]"){const t=Q.get(e);if(!t)throw te.create("no-app",{appName:e});return t}(),t){return ee(e,"database").getImmediate({identifier:t})}(function(e,t={}){if("object"!=typeof t){t={name:t}}const n=Object.assign({name:"[DEFAULT]",automaticDataCollectionEnabled:!1},t),i=n.name;if("string"!=typeof i||!i)throw te.create("bad-app-name",{appName:String(i)});const s=Q.get(i);if(s){if(S(e,s.options)&&S(n,s.config))return s;throw te.create("duplicate-app",{appName:i})}const r=new U(i);for(const e of X.values())r.addComponent(e);const o=new ne(e,n,r);return Q.set(i,o),o}({apiKey:"AIzaSyBSQ4EwRXTEF6Rs_3REQTWtcOoBu3HOKEQ",authDomain:"gritbook-social-media-app.firebaseapp.com",databaseURL:"https://gritbook-social-media-app-default-rtdb.europe-west1.firebasedatabase.app",projectId:"gritbook-social-media-app",storageBucket:"gritbook-social-media-app.appspot.com",messagingSenderId:"110931333305",appId:"1:110931333305:web:c540e52ab897acade50b68"}));class Vr{constructor(e,t){this.username=e,this.password=t}getUsername(){return this.username}getPassword(){return this.password}}const $r=document.querySelector("#travel"),Yr=document.querySelector("#sport"),Gr=document.querySelector("#gaming");let Kr=$r,Qr=!0,Xr=!1,Jr=!1;const Zr=e=>{"travel-forum"==e?(Kr=$r,$r.style.display="flex",Yr.style.display="none",Gr.style.display="none",Qr=!0,Xr=!1,Jr=!1,_o()):"sport-forum"==e?(Kr=Yr,$r.style.display="none",Yr.style.display="flex",Gr.style.display="none",Qr=!1,Xr=!0,Jr=!1,_o()):"gaming-forum"==e&&(Kr=Gr,$r.style.display="none",Yr.style.display="none",Gr.style.display="flex",Qr=!1,Xr=!1,Jr=!0,_o())},eo=e=>{const t=document.createElement("div"),n=document.createElement("div"),i=document.createElement("h5"),s=document.createElement("h5"),r=document.createElement("p"),o=document.createElement("button"),a=document.createElement("div");i.innerText=e.username,s.innerText=e.timestamp,r.innerText=e.message,o.innerText="X",o.className="delete-btn",t.className="user-div",n.className="mess-div",a.className="message-container",a.id=e.id,t.append(i),t.append(s),n.append(r),n.append(o),a.append(t),a.append(n),Kr.append(a),o.addEventListener("click",(()=>{console.log("delete button clicked"),console.log(e.id),console.log(Kr.id);const t=Rr(zr,"/messages/"+Kr.id+"/"+e.id);console.log(t),function(e){Ys("remove",e._path),Mr(e,null)}(t)}))};class to{constructor(e,t,n,i){this.id=e,this.username=t,this.message=n,this.timestamp=i}clearChat(){document.querySelector(`#${this.id}`).remove()}}const no=Rr(zr,"/messages/travel"),io=Rr(zr,"messages/sport"),so=Rr(zr,"messages/gaming"),ro=Rr(zr,"/users");let oo;const ao=document.querySelectorAll(".forum-btn"),lo=document.querySelector("#user"),ho=document.querySelector("#message"),co=document.querySelector("#add-message-btn");let uo=[];const po=()=>{!0===Qr?oo=no:!0===Xr?oo=io:!0===Jr&&(oo=so)},_o=()=>{po(),Ur(oo,(e=>{const t=e.val();console.log(t);for(const e of uo)e.clearChat();uo=[];for(const e in t)uo.push(new to(e,t[e].username,t[e].message,t[e].timestamp));console.log(uo);for(const e of uo)eo(e)}))},fo=()=>{let e;return Ur(ro,(t=>{e=t.val(),console.log(e)})),e};fo(),new class{constructor(){this.signInForm=document.querySelector("#login-form"),this.signUpForm=document.querySelector("#signup-form"),this.username=document.querySelector("#username"),this.password=document.querySelector("#password"),document.querySelector("#signin-btn").addEventListener("click",this.validator.bind(this)),document.querySelector("#btn-signup-ui").addEventListener("click",this.hideUI.bind(this))}validator(e){if(e.preventDefault(),console.log("validator invoked"),console.log(this.username.value),console.log(this.password.value),this.username.value.length>=4&&this.password.value.length>=4){const e=fo();if(e){for(const t in e)if(this.username.value==e[t].username&&this.password.value==e[t].password)return console.log("login success"),void this.clearForm();console.log("login failed"),this.clearForm()}}else console.log("username/password length is less than 4 characters"),this.clearForm()}hideUI(e){e.preventDefault(),this.clearForm(),this.signInForm.style.display="none",this.signUpForm.style.display="block"}clearForm(){this.signInForm.reset()}},new class{constructor(){this.usernameInField=document.querySelector("#new_username"),this.passwordInField=document.querySelector("#new_password"),this.repeatPasswordInField=document.querySelector("#repeat_password"),this.signUpform=document.querySelector("#signup-form"),this.signInForm=document.querySelector("#login-form"),document.querySelector("#signup-btn").addEventListener("click",this.validateForm.bind(this)),document.querySelector("#btn-display-signin").addEventListener("click",this.hideUI.bind(this))}validateForm(e){e.preventDefault();const t=fo();let n=[];for(const e in t)n.push(t[e].username);this.usernameInField.value.length<4||this.passwordInField.value.length<4||this.repeatPasswordInField.value.length<4?alert("username/password length is less than 4 characters"):n.includes(this.usernameInField.value)?alert("username already exists"):this.passwordInField.value==this.repeatPasswordInField.value?(alert("signup success"),this.initNewUser(new Vr(this.usernameInField.value,this.passwordInField.value))):alert("password does not match")}hideUI(e){e.preventDefault(),console.log("hideUI invoked"),this.signUpform.style.display="none",this.signInForm.style.display="block"}initNewUser(e){console.log("initNewUser invoked");const t={username:e.getUsername(),password:e.getPassword()},n=Rr(zr,"/users"),i={};i[Ar(n).key]=t,Fr(n,i),this.clearForm()}clearForm(){this.signUpform.reset()}},Zr("travel-forum"),ao.forEach((e=>{e.addEventListener("click",(e=>{const t=e.target.id;Zr(t)}))})),co.addEventListener("click",(e=>{e.preventDefault(),po();const t=Date.now(),n=new Date(t),i=e=>e<10?"0"+e:e,s=i(n.getDate())+"-"+i(n.getMonth()+1)+"-"+n.getFullYear()+", "+i(n.getHours())+":"+i(n.getMinutes());if(""==ho.value)alert("Enter text to post!");else{const e={message:ho.value,username:lo.innerText,timestamp:s};ho.value="";const t={};t[Ar(oo).key]=e,Fr(oo,t)}}));
//# sourceMappingURL=index.a81520fd.js.map
