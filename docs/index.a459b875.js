 ...
 * limitations under the License.
 */
ie("firebase","9.6.10","app");const zr=function(e=function(e="[DEFAULT]"){const t=Q.get(e);if(!t)throw te.create("no-app",{appName:e});return t}(),t){return ee(e,"database").getImmediate({identifier:t})}(function(e,t={}){if("object"!=typeof t){t={name:t}}const n=Object.assign({name:"[DEFAULT]",automaticDataCollectionEnabled:!1},t),i=n.name;if("string"!=typeof i||!i)throw te.create("bad-app-name",{appName:String(i)});const s=Q.get(i);if(s){if(S(e,s.options)&&S(n,s.config))return s;throw te.create("duplicate-app",{appName:i})}const r=new U(i);for(const e of X.values())r.addComponent(e);const o=new ne(e,n,r);return Q.set(i,o),o}({apiKey:"AIzaSyBSQ4EwRXTEF6Rs_3REQTWtcOoBu3HOKEQ",authDomain:"gritbook-social-media-app.firebaseapp.com",databaseURL:"https://gritbook-social-media-app-default-rtdb.europe-west1.firebasedatabase.app",projectId:"gritbook-social-media-app",storageBucket:"gritbook-social-media-app.appspot.com",messagingSenderId:"110931333305",appId:"1:110931333305:web:c540e52ab897acade50b68"}));class Vr{constructor(e,t){this.username=e,this.password=t}getUsername(){return this.username}getPassword(){return this.password}}const $r=document.querySelector("#travel"),Yr=document.querySelector("#sport"),Gr=document.querySelector("#gaming"),Kr=document.querySelector("#travel-chat"),Qr=document.querySelector("#sport-chat"),Xr=document.querySelector("#gaming-chat");let Jr=$r,Zr=Kr,eo=!0,to=!1,no=!1;const io=e=>{"travel-forum"==e?(Jr=$r,Zr=Kr,$r.style.display="flex",Yr.style.display="none",Gr.style.display="none",eo=!0,to=!1,no=!1,yo()):"sport-forum"==e?(Jr=Yr,Zr=Qr,$r.style.display="none",Yr.style.display="flex",Gr.style.display="none",eo=!1,to=!0,no=!1,yo()):"gaming-forum"==e&&(Jr=Gr,Zr=Xr,$r.style.display="none",Yr.style.display="none",Gr.style.display="flex",eo=!1,to=!1,no=!0,yo())},so=e=>{const t=document.createElement("div"),n=document.createElement("div"),i=document.createElement("h5"),s=document.createElement("h5"),r=document.createElement("p"),o=document.createElement("button"),a=document.createElement("div");i.innerText=e.username,s.innerText=e.timestamp,r.innerText=e.message,o.innerText="X",o.className="delete-btn",t.className="user-div",n.className="mess-div",a.className="message-container",a.id=e.id,t.append(i),t.append(s),n.append(r),n.append(o),a.append(t),a.append(n),Zr.prepend(a),Jr.append(Zr),o.addEventListener("click",(()=>{console.log("delete button clicked"),console.log(e.id),console.log(Jr.id);const t=Rr(zr,"/messages/"+Jr.id+"/"+e.id);console.log(t),function(e){Ys("remove",e._path),Mr(e,null)}(t)}))};class ro{constructor(e,t,n,i){this.id=e,this.username=t,this.message=n,this.timestamp=i}clearChat(){document.querySelector(`#${this.id}`).remove()}}const oo=Rr(zr,"/messages/travel"),ao=Rr(zr,"messages/sport"),lo=Rr(zr,"messages/gaming"),ho=Rr(zr,"/users");let co;const uo=document.querySelectorAll(".forum-btn"),po=document.querySelector(".user"),_o=document.querySelector("#message"),fo=document.querySelector("#add-message-btn");let go=[];const mo=()=>{!0===eo?co=oo:!0===to?co=ao:!0===no&&(co=lo)},yo=()=>{mo(),Ur(co,(e=>{const t=e.val();console.log(t);for(const e of go)e.clearChat();go=[];for(const e in t)go.push(new ro(e,t[e].username,t[e].message,t[e].timestamp));console.log(go);for(const e of go)so(e)}))},vo=()=>{let e;return Ur(ho,(t=>{e=t.val(),console.log(e)})),e};vo(),new class{constructor(){this.signInForm=document.querySelector("#login-form"),this.signUpForm=document.querySelector("#signup-form"),this.username=document.querySelector("#username"),this.password=document.querySelector("#password"),document.querySelector("#signin-btn").addEventListener("click",this.validator.bind(this)),document.querySelector("#btn-signup-ui").addEventListener("click",this.hideUI.bind(this))}validator(e){if(e.preventDefault(),console.log("validator invoked"),console.log(this.username.value),console.log(this.password.value),this.username.value.length>=4&&this.password.value.length>=4){const e=vo();if(e){for(const t in e)if(this.username.value==e[t].username&&this.password.value==e[t].password)return console.log("login success"),void this.clearForm();console.log("login failed"),this.clearForm()}}else console.log("username/password length is less than 4 characters"),this.clearForm()}hideUI(e){e.preventDefault(),this.clearForm(),this.signInForm.style.display="none",this.signUpForm.style.display="flex"}clearForm(){this.signInForm.reset()}},new class{constructor(){this.usernameInField=document.querySelector("#new_username"),this.passwordInField=document.querySelector("#new_password"),this.repeatPasswordInField=document.querySelector("#repeat_password"),this.signUpform=document.querySelector("#signup-form"),this.signInForm=document.querySelector("#login-form"),document.querySelector("#signup-btn").addEventListener("click",this.validateForm.bind(this)),document.querySelector("#btn-display-signin").addEventListener("click",this.hideUI.bind(this))}validateForm(e){e.preventDefault();const t=vo();let n=[];for(const e in t)n.push(t[e].username);this.usernameInField.value.length<4||this.passwordInField.value.length<4||this.repeatPasswordInField.value.length<4?alert("username/password length is less than 4 characters"):n.includes(this.usernameInField.value)?alert("username already exists"):this.passwordInField.value==this.repeatPasswordInField.value?(alert("signup success"),this.initNewUser(new Vr(this.usernameInField.value,this.passwordInField.value))):alert("password does not match")}hideUI(e){e.preventDefault(),console.log("hideUI invoked"),this.signUpform.style.display="none",this.signInForm.style.display="flex"}initNewUser(e){console.log("initNewUser invoked");const t={username:e.getUsername(),password:e.getPassword()},n=Rr(zr,"/users"),i={};i[Ar(n).key]=t,Fr(n,i),this.clearForm()}clearForm(){this.signUpform.reset()}},io("travel-forum"),uo.forEach((e=>{e.addEventListener("click",(e=>{const t=e.target.id;io(t)}))})),fo.addEventListener("click",(e=>{e.preventDefault(),mo();const t=Date.now(),n=new Date(t),i=e=>e<10?"0"+e:e,s=i(n.getDate())+"-"+i(n.getMonth()+1)+"-"+n.getFullYear()+", "+i(n.getHours())+":"+i(n.getMinutes());if(""==_o.value)alert("Enter text to post!");else{const e={message:_o.value,username:po.innerText,timestamp:s};_o.value="";const t={};t[Ar(co).key]=e,Fr(co,t)}}));
//# sourceMappingURL=index.a459b875.js.map
